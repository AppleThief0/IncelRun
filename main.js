...
function createBuilding() {
  ...
  // 👉 Add name label
  const names = ["Nadia", "Siffat", "Fabiha"];
  const randomName = names[Math.floor(Math.random() * names.length)];
  const nameLabel = document.createElement("div");
  nameLabel.textContent = randomName;
  ...
  nameLabel.style.top = "-20px"; // Place name above the building
  building.style.position = "absolute"; // Needed for name positioning
  building.appendChild(nameLabel);
...
function updateBackground(name) {
  if (
    name.toLowerCase() == "pookie" 
  ) {
    gameBg.style.backgroundImage = "url('./assets/images/bg-pookiee.jpg')";
    ...
    bird.style.backgroundImage = "url('./assets/images/nigga.png')";
    bgMusic.pause();
  }
}
